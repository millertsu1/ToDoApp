...
function Todos() {
  ...
  const addTodo = (text) => {
    ...
    Swal.fire({
      ...
      text: `Se ha agregado la tarea "${text}" a la lista.`,
      timer: 3000,
  ...
  const deleteTodo = (index) => {
    ...
    Swal.fire({
      ...
      text: `Se ha eliminado la tarea "${deleteTodo}" de la lista.`,
      timer: 3000,
      showConfirmButton: false,
    });
  };
    ...
    <div className="flex flex-col mt-20 max-w-[500px] w-full px-6 sm:px-0 z-50">
      <header className="flex justify-between items-center mb-16">
        <div>
          ...
          <h1 className="sm:text-4xl text-3xl capitalize text-white tracking-[.50em] text-left ">
          <div>
          <ThemeToggler />
        </div>
            ToDo List APP
      ...
      >
        ...
        <input
          ...
          placeholder="crea una nueva tarea y da enter para agregar"
        />
      </form>
      <div className="text-[#9293a4] text-sm text-center dark:text-[#777b93]">
            Selecciona y arrastra para reordenar la lista
          </div>
